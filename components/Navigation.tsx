    ...
    <>
      ...
      >
        <div className="container-luxury flex items-center justify-between">
          ...
          {/* Desktop nav */}
          <nav className="hidden lg:flex items-center gap-7" ref={dropdownRef} aria-label="Navigare principală">
            {navLinks.map((link) => (
              link.children ? (
                <div key={link.label} className="relative">
                  <button
                    onClick={() => setActiveDropdown(activeDropdown === link.label ? null : link.label)}
                    className="flex items-center gap-1 text-[0.65rem] font-sans font-medium tracking-widest uppercase text-cream-400 hover:text-gold-600 transition-colors duration-300"
                      ...
                      >
                        {link.children.map((child) => (
                          <Link
                            key={child.href}
                            href={child.href}
                            className="block px-5 py-3 text-[0.65rem] font-sans font-medium tracking-widest uppercase text-cream-400 hover:text-gold-600 hover:bg-gold-600/5 transition-all duration-200"
                          >
                            {child.label}
                          </Link>
                        ))}
                ...
                <Link
                  key={link.label}
                  href={link.href}
                  className={`text-[0.65rem] font-sans font-medium tracking-widest uppercase transition-colors duration-300 ${
                    pathname === link.href
                      ? 'text-gold-600'
            ...
            <Link href="/contact" className="btn-gold text-[0.6rem] px-5 py-3">
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto py-6 px-6">
              {navLinks.map((link, i) => (
                ...
                >
                  {link.children ? (
                    <div className="mb-2">
                      <span className="block py-3 text-[0.6rem] font-sans font-medium tracking-widest uppercase text-gold-600/70 border-b border-gold-600/10">
                        {link.label}
                      </span>
                      <div className="pl-4 mt-1">
                        {link.children.map((child) => (
                          <Link
                            key={child.href}
                            href={child.href}
                            className="block py-2.5 text-[0.6rem] font-sans tracking-widest uppercase text-cream-400 hover:text-gold-600 transition-colors"
                          >
                            {child.label}
                          </Link>
                        ))}
                    ...
                    <Link
                      href={link.href}
                      className={`block py-3 text-[0.6rem] font-sans font-medium tracking-widest uppercase border-b border-gold-600/10 transition-colors ${
            ...
            <div className="px-6 py-6 border-t border-gold-600/10 space-y-4">
              <Link href="/contact" className="btn-gold w-full text-center text-xs justify-center">
