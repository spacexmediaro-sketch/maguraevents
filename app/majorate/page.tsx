import type { Metadata } from 'next';
import AnimatedSection from '@/components/AnimatedSection';
import Link from 'next/link';

export const metadata: Metadata = {
  title: 'Majorate — Petrecere de 18 Ani Spectaculoasă | Măgura Events Buzău',
  description:
    'Organizăm petreceri de majorat spectaculoase în saloanele premium din Buzău. DJ profesionist, lumini de scenă, pistă de dans, photobox și tot ce ai nevoie pentru cea mai tare petrecere de 18 ani.',
  keywords: ['majorat Buzău', 'petrecere majorat', 'salon majorat', 'organizare majorat', 'petrecere 18 ani'],
  alternates: { canonical: 'https://magura-events.ro/majorate' },
};

export default function MajoratePage() {
  return (
    <>
      <section
        className="pt-40 pb-24 relative"
        style={{
          background: `radial-gradient(ellipse 60% 60% at 50% 0%, rgba(139,105,20,0.12) 0%, transparent 60%), #080808`,
        }}
      >
        <div className="container-luxury text-center">
          <AnimatedSection>
            <p style={{ fontSize: '2.5rem' }} className="mb-4" role="img" aria-label="Majorat">🎓</p>
            <p className="label-gold mb-4">✦ 18 ani — o seară de neuitat ✦</p>
            <h1 className="heading-xl text-cream-200 mb-6">
              Majorate{' '}
              <span className="text-gold-gradient italic">spectaculoase</span>
            </h1>
            <span className="gold-line" />
            <p className="font-sans text-cream-500 max-w-2xl mx-auto mt-6" style={{ lineHeight: 1.9 }}>
              Petrecerea de majorat trebuie să fie legendară. La Măgura Events creăm experiențe unice pentru tinerii care celebrează 18 ani — cu energie, muzică, lumini și o atmosferă de club premium.
            </p>
          </AnimatedSection>
        </div>
      </section>

      <section className="section-padding" style={{ background: '#0a0a0a' }}>
        <div className="container-luxury max-w-5xl">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {[
              { emoji: '🎵', title: 'DJ Profesionist', desc: 'Mixuri live, lumini de club și o pistă de dans care nu se mai oprește toată noaptea.' },
              { emoji: '📸', title: 'Photobox & Reels', desc: 'Colțișor foto premium cu accesorii tematice și imprimare instant a pozelor.' },
              { emoji: '🍹', title: 'Bar Open', desc: 'Bar complet cu băuturi diverse, servit de barmani profesioniști pe toată durata petrecerii.' },
              { emoji: '🎂', title: 'Tort Spectaculos', desc: 'Tort de majorat tematic, personalizat în funcție de personalitatea aniversatului.' },
              { emoji: '🎆', title: 'Momente Speciale', desc: 'Confetti, baloane, surprize personalizate și momente de neuitat pentru aniversat.' },
              { emoji: '🎭', title: 'Decor Tematic', desc: 'Decoruri complet personalizate — de la glam și auriu la teme specifice cerute de aniversat.' },
            ].map((item) => (
              <AnimatedSection key={item.title}>
                <div className="card-luxury p-6 text-center h-full">
                  <span style={{ fontSize: '2rem' }} className="block mb-3" role="img" aria-label={item.title}>{item.emoji}</span>
                  <h3 className="font-serif text-xl text-cream-100 mb-3">{item.title}</h3>
                  <p className="font-sans text-xs text-cream-500" style={{ lineHeight: 1.75 }}>{item.desc}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>

          <AnimatedSection className="text-center">
            <Link href="/contact" className="btn-gold"><span>Solicită Ofertă Majorat</span></Link>
          </AnimatedSection>
        </div>
      </section>
    </>
  );
}
